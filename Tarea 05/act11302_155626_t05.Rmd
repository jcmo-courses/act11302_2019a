...
antiguedad <- datos$Calendar_Year-datos$Model_Year
...
datos[,"Categoria"] <- 1
datos[which(datos$Antiguedad<=3),"Categoria"] <- 2
datos[which(datos$Antiguedad>3),"Categoria"] <- 3
datos[which(datos$Antiguedad==0),"Categoria"] <- 1
```
...
datos1=datos[which(datos$Categoria==1),]
...
datos2=datos[which(datos$Categoria==2),]
...
datos3=datos[which(datos$Categoria==3),]
...
Notemos que los parametros de cada año son muy parecidos y por ello si podriamos asumir homogeneidad en los moelos de cada año
