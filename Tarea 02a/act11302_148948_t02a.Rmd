...
#Función BinNeg.Plot
Crear función BinNeg.Plot() modificando la función Poisson.Plot() para generar resultados analogos 
...
```{r}
Binomial.Plot(30,.7, quantile = 0.7)
```
...
BinNeg.Plot <- function(n, p, low=0, high=n,scale = F, a=NA,b=NA,calcProb=!all(is.na(c(a,b))),quantile=NA,calcQuant=!is.na(quantile)){
  #Binomial Negativa
  sd = (sqrt(n*(1-p)))/p
  low = max(0, round(n * (1-p) -5 * sd))
  high = min(n, round(n * (1-p) + 5 * sd))
  values = low:high
