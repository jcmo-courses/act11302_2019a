# Tarea 2 - Calculo Actuarial III
# BinNeg.Plot, Binomial.Plot, Poisson.Plot
# Fabrizzio Perez Aceituno (156165)
...
BinNeg.Plot <- function(successes,proba, a=NA,b=NA,calcProb=(!is.na(a) | !is.na(b)),quantile=NA,calcQuant=!is.na(quantile)){
  ...
  plot(c(low,high), c(0,max(probs)), type = "n", 
       ...
       main = "Binomial Negativa", lwd=2)
  lines(values, probs, type = "h", col = "black", lwd=2)
  ...
  if(calcProb) {
    ...
    if(is.na(b)){
      ...
      title(paste("P(",a," <= Y ) = ",round(prob,6),sep=""),line=0,col.main="dark orange", lwd=2)
      ...
      title(paste("P(",a," <= N <= ",b,") = ",round(prob,6),sep=""),line=0,col.main="dark orange", lwd=2)
      u = seq(max(c(a,low)),min(c(b,high)),by=1)
    }
    v = dnbinom(u,successes,proba)
    lines(u,v,type="h",col="dark orange", lwd=2)
  }
  else if(calcQuant==T) {
    if(quantile < 0 || quantile > 1)
      stop("El cuantil debe estar entre 0 y 1")
    x = qnbinom(quantile,successes,proba)
    title(paste("",quantile," quantile = ",x,sep=""),line=0,col.main="dark orange", lwd=2)
    u = 0:x
    v = dnbinom(u,successes,proba)
    lines(u,v,type="h",col="dark orange", lwd=2)
  }
